...
tbl %>%
  ...
  ggplot() + 
    ...
    labs(x = "minutes at 65 °C",
         y = expression( "RT-LAMP (ΔOD"["30 min"]~")" ) ) -> panel_a
...
tbl_wide %>%
  ...
  ggplot() +
    ...
    labs(x = expression( "RT-LAMP ( ΔOD"["30 min"] - "ΔOD"["10 min"]~")" ),
         y = expression( "RT-LAMP ( ΔOD"["30 min"]~")" )) -> panel_b

fig_layout <- '
AAAAA
BBBBC
'
panel_a + panel_b + guide_area() +
  plot_layout(design = fig_layout, guides = "collect") +
  plot_annotation(tag_levels = "A")

ggsave("Figure_6.png", width=20, height=20, units="cm", dpi=300)
ggsave("SVGs/Figure_6.svg", width=20, height=20, units="cm")
